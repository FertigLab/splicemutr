...
HLA_files <- "/media/theron/My_Passport/head_and_neck_DARIA/data/splicemutr_05_26_2021/processed_data/class1/%s_tx_dict_summary_perc.txt"
...
kmer_dat <- data.frame(matrix(0,nrow=nrow(splicemutr_dat),ncol=length(genotypes)))
kmer_vec <- vector("list",nrow(splicemutr_dat))
...
for (i in seq(1,length(arcas_file))){
  ...
  for (allele in class1_alleles){
    ...
    kmers <- lapply(seq(length(counts)),function(i){
      ...
      kmers <- all_kmers
      kmer_vec[[rows[i]]] <<- unique(c(kmer_vec[[rows[i]]], kmers))
      return(T)
    })
  }
  kmer_string <- vapply(seq(length(kmer_vec)),function(i){
    paste(kmer_vec[[i]],collapse=":")
  },character(1))
  kmer_dat[,i]<-kmer_string
}
colnames(kmer_dat)<-arcas_file

saveRDS(kmer_dat,file="/media/theron/My_Passport/head_and_neck_DARIA/data/splicemutr_05_26_2021/processed_data/class1/kmer_dat.rds")
write.table(kmer_dat,
            file="/media/theron/My_Passport/head_and_neck_DARIA/data/splicemutr_05_26_2021/processed_data/class1/kmer_dat.txt",
