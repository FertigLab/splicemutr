...
# Looking at immunity per comparison psi vs kmers per sample

```{r}

comparisons <- readRDS("/media/theron/My_Passport/Valsamo/analysis/leafcutterMD/comparisons.rds")
comp_dir <- "/media/theron/My_Passport/Valsamo/analysis/splicemutr_output"

for (comp in names(comparisons)){
  psi_all <- readRDS(sprintf("%s/psi_all_%s.rds",comp_dir,comp))
  kmers_specific <- readRDS(sprintf("%s/kmers_specific_%s.rds",comp_dir,comp))
  kmers_specific <- as.data.frame(t(kmers_specific))
  splice_dat <- readRDS(sprintf("%s/splice_dat_%s.rds",comp_dir,comp))
  splice_dat_filt <- splice_dat %>% dplyr::filter(!(annotated == "annotated" & modified == "changed"))
  # psi_all_filt <- psi_all %>% dplyr::filter(comp_juncs_parsed %in% splice_dat_filt$juncs)
  kmers_specific_filt <- kmers_specific %>% dplyr::filter(V2 %in% splice_dat_filt$juncs)
  # psi_all_filt <- psi_all_filt[kmers_specific_filt$V2,]
  # 
  # for (col in seq(2,ncol(psi_all_filt))){
  #   psi_all_filt[,col]<-as.numeric(psi_all_filt[,col])
  # }
  for (col in seq(3,ncol(kmers_specific_filt))){
    kmers_specific_filt[,col]<-as.numeric(kmers_specific_filt[,col])
  }
  junc_dat <- unique(kmers_specific_filt$V2)
  kmers_specific_filt_junc <- data.frame(t(vapply(junc_dat,function(junc){
    kmers_small <- kmers_specific_filt %>% dplyr::filter(V2 == junc)
    return(apply(kmers_small[seq(3,ncol(kmers_small))],2,mean))
  },numeric(ncol(kmers_specific_filt)-2))))
  tar_cols <- which(str_detect(colnames(psi_all[seq(2,ncol(psi_all))]),"tar"))
  kmers_specific_filt_junc_spec <- data.frame(apply(kmers_specific_filt_junc[,tar_cols],1,mean))
  kmers_specific_filt_junc_spec$juncs <- junc_dat
  colnames(kmers_specific_filt_junc_spec)<-c("kmer_count","junc")
  print(pheatmap::pheatmap(log10(kmers_specific_filt_junc+1),cluster_rows = F,cluster_cols = F,show_rownames = F,show_colnames = F,main=comp))
}
