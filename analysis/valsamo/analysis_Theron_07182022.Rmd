...
response_summ_all$resp <- factor(response_summ_all$response,levels=c("CR_PR","SD","PD"))

wilcox_test_val_all <- compare_means(gene_metric ~ time, 
                         comparisons = my_comparisons, p.adjust.method = "BH", data=response_summ_all,group.by = c("response","treat"))

ggplot(response_summ_all,aes(x=time,y=gene_metric,fill=resp))+
  geom_violin()+
  facet_grid(treat~response)+
  add_pvalue(wilcox_test_val_all, label = "p.signif", remove.bracket = TRUE,y.position=0.4)+
  ylab("Splicing Antigenicity")+
  xlab("Treatment Time")
...
wilcox_test_val_all <- compare_means(gene_metric ~ response, 
                         comparisons = my_comparisons, p.adjust.method = "BH", data=response_summ_all,group.by = c("treat","time"))
wilcox_test_val_all$y.position <- c(0.4, 0.35, 0.3,0.4, 0.35, 0.3,0.4, 0.35, 0.3,0.4, 0.35, 0.3,0.4, 0.35, 0.3,0.4, 0.35, 0.3)
ggplot(response_summ_all,aes(x=response,y=gene_metric,fill=resp))+
  geom_violin()+
...
splicing_antigenicity_PRE_POST$response <- factor(splicing_antigenicity_PRE_POST$response,levels=c("CR_PR","SD","PD"))

ggplot(splicing_antigenicity_PRE_POST,aes(y=log2(POST_ov_PRE),x=subject,fill=response))+
  geom_violin()+
  # facet_grid(treatment~.)+
...
ggplot(splicing_antigenicity_PRE_POST_NIV1_IPI3,aes(y=log2(POST_ov_PRE),x=subject,fill=response))+
  geom_violin()+
  geom_hline(yintercept=0)+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
ggplot(splicing_antigenicity_PRE_POST_NIV3_PROG,aes(y=log2(POST_ov_PRE),x=subject,fill=response))+
  geom_violin()+
  geom_hline(yintercept=0)+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
