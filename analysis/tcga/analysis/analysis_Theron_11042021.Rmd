...
# Extracting samples in top 10 % splicing antigenicity

```{r}

# ggplot(splice_mut_per_sample_all,aes(x=splice_mut_av))+geom_histogram()
splice_mut_av_perc <- quantile(splice_mut_per_sample_all$splice_mut_av,0.90)
samples_av <- splice_mut_per_sample_all[which(splice_mut_per_sample_all$splice_mut_av >= splice_mut_av_perc),"sample"]
# ggplot(splice_mut_per_sample_all,aes(x=splice_mut_med))+geom_histogram()
splice_mut_med_perc <- quantile(splice_mut_per_sample_all$splice_mut_med,0.90)
samples_med <- splice_mut_per_sample_all[which(splice_mut_per_sample_all$splice_mut_med >= splice_mut_med_perc),"sample"]
# ggplot(splice_mut_per_sample_all,aes(x=splice_mut_max))+geom_histogram()
splice_mut_max_perc <- quantile(splice_mut_per_sample_all$splice_mut_max,0.90)
samples_max <- splice_mut_per_sample_all[which(splice_mut_per_sample_all$splice_mut_max >= splice_mut_max_perc),"sample"]
...
splice_maf_data_fill[,c("splice_ant_av","splice_ant_med","splice_ant_max")] <- data.frame(t(vapply(splice_maf_data_fill$Tumor_Sample_ID,function(ID){
  a<-which(splice_mut_per_sample_all$sample == ID)
  if (length(a)==0){
    return(c(0,0,0))
  } else {
    return(as.numeric(splice_mut_per_sample_all[a,c("splice_mut_av","splice_mut_med","splice_mut_max")]))
  }
},numeric(3))))
...
sample_cancer_dat <- splice_mut_per_sample_all
a<-lapply(sample_cancer_dat$sample,function(ID){
  splice_maf_data_fill_small <- splice_maf_data_fill %>% dplyr::filter(Tumor_Sample_ID == ID)
  if (nrow(splice_maf_data_fill_small) == 0){
    a<-rep(0,length(splicing_factor_genes))
    names(a)<-splicing_factor_genes
    return(a)
  }
...
cancer_order <- order(sample_cancer_dat$cancer)
sample_splice_factor_dat_muts <- as.matrix(sample_splice_factor_dat[cancer_order,])
sample_cancer_dat <- sample_cancer_dat[cancer_order,]
av_to_keep <- which(sample_cancer_dat$sample %in% samples_av)
med_to_keep <- which(sample_cancer_dat$sample %in% samples_med)
max_to_keep <- which(sample_cancer_dat$sample %in% samples_max)
...
sample_splice_factor_dat_muts_av <- sample_splice_factor_dat_muts[av_to_keep,]
sample_cancer_dat_av <- sample_cancer_dat[av_to_keep,]
Heatmap(sample_splice_factor_dat_muts_av,
        right_annotation = rowAnnotation(spliceant = anno_barplot(sample_cancer_dat_av$splice_mut_av)),
        left_annotation = rowAnnotation(cancer = sample_cancer_dat_av$cancer),
        show_row_names=F,
        show_column_names = F,
        cluster_rows=T,
        cluster_columns=T)
...
sample_splice_factor_dat_muts_med <- sample_splice_factor_dat_muts[med_to_keep,]
sample_cancer_dat_med <- sample_cancer_dat[med_to_keep,]

Heatmap(sample_splice_factor_dat_muts_med,
        right_annotation = rowAnnotation(spliceant = anno_barplot(sample_cancer_dat_med$splice_mut_med)),
        left_annotation = rowAnnotation(cancer = sample_cancer_dat_med$cancer),
        show_row_names=F,
        show_column_names = F,
        cluster_rows=T,
        cluster_columns=T)
...
sample_splice_factor_dat_muts_max <- sample_splice_factor_dat_muts[max_to_keep,]
sample_cancer_dat_max <- sample_cancer_dat[max_to_keep,]

Heatmap(sample_splice_factor_dat_muts_max,
        right_annotation = rowAnnotation(spliceant = anno_barplot(sample_cancer_dat_max$splice_mut_max)),
        left_annotation = rowAnnotation(cancer = sample_cancer_dat_max$cancer),
        show_row_names=F,
        show_column_names = F,
        cluster_rows=T,
        cluster_columns=T)
...
# Reading in and saving gene expression data
...
a<-create_rse(proj_info[5,],type="gene")
gene_expression <- a@assays@data@listData$raw_counts
