...
for (i in seq(length(cancers))){
  ...
  a<-cor.test(combined_gene_metric_per_sample$splicing_antigenicity_norm,log10(combined_gene_metric_per_sample$non_silent_mutations_per_mb+1),method="kendall")
  TMB_all_cor[cancer,"SA_vs_TMB_cor"]<-a$estimate
  TMB_all_pvals[cancer,"SA_vs_TMB_pval"]<-a$p.value

  a<-cor.test(combined_gene_metric_per_sample$splicing_antigenicity_norm,combined_gene_metric_per_sample$leukocyte_fraction,method="kendall")
  TMB_all_cor[cancer,"SA_vs_leuk_frac_cor"]<-a$estimate
  TMB_all_pvals[cancer,"SA_vs_leuk_frac_pval"]<-a$p.value

  a<-cor.test(combined_gene_metric_per_sample$splicing_antigenicity_norm,log10(combined_gene_metric_per_sample$numberOfImmunogenicMutation+1),method="kendall")
  TMB_all_cor[cancer,"SA_vs_numImmMut_cor"]<-a$estimate
  TMB_all_pvals[cancer,"SA_vs_numImmMut_pval"]<-a$p.value
  
  a<-cor.test(combined_gene_metric_per_sample$splicing_antigenicity_norm,log10(combined_gene_metric_per_sample$numberOfBindingExpressedPMHC+1),method="kendall")
  TMB_all_cor[cancer,"SA_vs_numBindExprPMHC_cor"]<-a$estimate
  TMB_all_pvals[cancer,"SA_vs_numBindExprPMHC_pval"]<-a$p.value

  a<-cor.test(combined_gene_metric_per_sample$splicing_antigenicity_norm,log2(combined_gene_metric_per_sample$num_TCR_Clones+1),method="kendall")
  ...
  for (cell in actual_cibersort_cells){
    a<-cor.test(as.numeric(cibersort_per_samp_df$splicing_antigenicity_norm),as.numeric(cibersort_per_samp_df[,cell]),method="kendall")
...
for (cancer in cancers){
  ...
  if (HIGH_COUNT > 0 & LOW_COUNT > 0){
    ...
    wilcox_test_val <- compare_means(splicing_antigenicity_norm ~ TMB_TYPE, 
...
print(ggplot(combined_gene_metric_per_sample_all_filt,aes(y=splicing_antigenicity_norm,x=TMB_TYPE))+
  geom_boxplot(fill="grey")+stat_pvalue_manual(wilcox_test_val, 
  y.position = max(combined_gene_metric_per_sample_all_filt$splicing_antigenicity_norm)+(max(combined_gene_metric_per_sample_all_filt$splicing_antigenicity)/2),
  label = "p.adj" )+labs(title=sprintf("all cohorts, HIGH: %d, LOW: %d",HIGH_COUNT,LOW_COUNT))+
    ylab(("splicing antigenicity")))

ggplot(combined_gene_metric_per_sample_all_filt,aes(x=splicing_antigenicity_norm,y=log10(non_silent_mutations_per_mb+1)))+
  geom_point()+
...
for (cancer in combined_gene_metric_per_sample_all$cancer){
  ...
  combined_gene_metric_per_sample_all_sum[combined_gene_metric_per_sample_all_sum$cancer==cancer,"splicing_antigenicity_norm"] <- mean(combined_gene_metric_per_sample_small$splicing_antigenicity_norm,na.rm=T)
}
ggplot(combined_gene_metric_per_sample_all_sum,aes(x=splicing_antigenicity_norm,y=median_TMB,label=cancer))+
  stat_cor(method = "kendall",label.x.npc="middle",label.y.npc="top")+
  geom_smooth(method="lm")+
...
print(Heatmap(t(intron_counts_per_cancer_scaled),
        ...
        bottom_annotation = HeatmapAnnotation(splicing_antigenicity_norm=anno_barplot(combined_gene_metric_per_sample_all_sum[rownames(intron_counts_per_cancer),"splicing_antigenicity_norm"])),
        show_row_names=T,
        show_column_names = T,
        cluster_rows=T,
        cluster_columns=T,
...
for (cancer in cancers){
    ...
    if (MUT_VALS>0 & WT_VALS>0){
      wixoc_test_val <- compare_means(splicing_antigenicity_norm ~ SF3B1,comparisons = my_comparisons, p.adjust.method = "BH", data=combined_gene_metric_per_sample_all_small)
      print(ggplot(combined_gene_metric_per_sample_all_small,aes(y=splicing_antigenicity_norm,x=SF3B1))+
      geom_boxplot(fill="grey")+
        stat_pvalue_manual(wixoc_test_val, 
    y.position = max(combined_gene_metric_per_sample_all_small$splicing_antigenicity_norm)+(max(combined_gene_metric_per_sample_all_small$splicing_antigenicity)/2),
    label = "p.adj" )+
    ...
    if (MUT_VALS>0 & WT_VALS>0){
      wixoc_test_val <- compare_means(splicing_antigenicity_norm ~ U2AF1,comparisons = my_comparisons, p.adjust.method = "BH", data=combined_gene_metric_per_sample_all_small)
      print(ggplot(combined_gene_metric_per_sample_all_small,aes(y=splicing_antigenicity_norm,x=U2AF1))+
      geom_boxplot(fill="grey")+
        stat_pvalue_manual(wixoc_test_val, 
    y.position = max(combined_gene_metric_per_sample_all_small$splicing_antigenicity_norm)+(max(combined_gene_metric_per_sample_all_small$splicing_antigenicity)/2),
    label = "p.adj" )+
    ...
    if (MUT_VALS>0 & WT_VALS>0){
      wixoc_test_val <- compare_means(splicing_antigenicity_norm ~ SRSF2,comparisons = my_comparisons, p.adjust.method = "BH", data=combined_gene_metric_per_sample_all_small)
      print(ggplot(combined_gene_metric_per_sample_all_small,aes(y=splicing_antigenicity_norm,x=SRSF2))+
        geom_boxplot(fill="grey")+
        stat_pvalue_manual(wixoc_test_val, 
    y.position = max(combined_gene_metric_per_sample_all_small$splicing_antigenicity_norm)+(max(combined_gene_metric_per_sample_all_small$splicing_antigenicity)/2),
    label = "p.adj" )+
    ...
    if (MUT_VALS>0 & WT_VALS>0){
      wixoc_test_val <- compare_means(splicing_antigenicity_norm ~ RBM10,comparisons = my_comparisons, p.adjust.method = "BH", data=combined_gene_metric_per_sample_all_small)
      print(ggplot(combined_gene_metric_per_sample_all_small,aes(y=splicing_antigenicity_norm,x=RBM10))+
      geom_boxplot(fill="grey")+
        stat_pvalue_manual(wixoc_test_val,
    y.position = max(combined_gene_metric_per_sample_all_small$splicing_antigenicity_norm)+(max(combined_gene_metric_per_sample_all_small$splicing_antigenicity)/2),
    label = "p.adj" )+
...
for (i in seq(length(cancers))){
  ...
  if (MUT_VALS>0 & WT_VALS>0){
    wixoc_test_val <- compare_means(splicing_antigenicity_norm ~ all_genes,comparisons = my_comparisons, p.adjust.method = "BH", data=combined_gene_metric_per_sample_all_small)
    MUT_SA <- combined_gene_metric_per_sample_all_small[combined_gene_metric_per_sample_all_small$all_genes=="MUT","splicing_antigenicity_norm"]
    WT_SA <- combined_gene_metric_per_sample_all_small[combined_gene_metric_per_sample_all_small$all_genes=="WT","splicing_antigenicity_norm"]
  ...
  for (j in seq(length(splicing_factor_genes))){
    ...
    gene_metric_av_gene_df <- data.frame(splicing_antigenicity_norm=combined_gene_metric_per_sample_all_small$splicing_antigenicity_norm,
    ...
    if (length(unique(gene_metric_av_gene_df$gene))==1){
      a<-c(rep(NA,8),cancer,gene,NA,
           length(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="WT"]),
                  length(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="MUT"]))
    } else {
      a<-compare_means(splicing_antigenicity_norm~gene,data=gene_metric_av_gene_df)
      a$cancer <- cancer
      a$gene <- gene
      a$mut_ov_wt <- mean(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="MUT"],na.rm=T)/mean(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="WT"],na.rm=T)
      a$WT_NUM <- length(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="WT"])
      a$MUT_NUM <- length(gene_metric_av_gene_df$splicing_antigenicity_norm[gene_metric_av_gene_df$gene=="MUT"])
      a<-data.frame(a)
...
for (j in seq(length(splicing_factor_genes))){
  gene <- splicing_factor_genes[j]
  gene_metric_av_gene_df <- data.frame(splicing_antigenicity_norm=combined_gene_metric_per_sample_all$splicing_antigenicity_norm,
