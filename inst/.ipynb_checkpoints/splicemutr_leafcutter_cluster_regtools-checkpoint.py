...
def pool_junc_reads(flist, options):
    ...
    for chrom in by_chrom:
        read_ks = [k for k, v in by_chrom[chrom].items() if v >= 10]  # a junction must have at least 3 reads
        read_ks.sort()
...
def sort_junctions(libl, options):
    ...
    if options.cluster == None:
        ...
        # refined_cluster = "%s_refined"%(outPrefix)
    else:
        ...
        for lib in merges[libN]:
            ...
            for ln in open(lib):
                ...
                if len(lnsplit) < 6:
                    sys.stderr.write("Error in %s \n" % lib)
                    continue
                chrom, A, B, dot, count, strand = lnsplit
