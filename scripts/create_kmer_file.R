...
full_kmers[small_splice_proto$rows,names(genotypes)] <- filler_mat
full_splice <- full_splice %>% dplyr::filter(!(verdict == "annotated" & modified == "changed") & deltapsi > 0 & protein_coding=="Yes")
