...
splicemutr_files <- read.table(splicemutr_file)
for (j in seq(length(splicemutr_files$V1))){
  if (j == 1){
    combined_gene_metric_normal <- readRDS(new_path(splicemutr_files$V1[j],
                                                    sprintf("%s/%s/GENE_METRIC_CP",TCGA_ROOT_DIR,cancer)))
  } else {
    a<-readRDS(new_path(splicemutr_files$V1[j],
                        sprintf("%s/%s/GENE_METRIC_CP",TCGA_ROOT_DIR,cancer)))
...
normal_all <- combined_gene_metric_normal[conglomerate_genes,
                                          normal_samples[normal_samples %in% colnames(combined_gene_metric_normal)]] # filtering by available normal samples and the unique set of conglomerate genes
...
tumor_all <- combined_gene_metric_tumor[conglomerate_genes,tumor_samples[tumor_samples %in% colnames(combined_gene_metric_tumor)]] # filtering by available tumor samples and the unique set of conglomerate genes
...
combined_gene_metric_tumor <- combined_gene_metric_tumor[conglomerate_genes,] # filtering the non-linearized tumor splicing antigenicity by conglomerate and differential genes
combined_gene_metric_normal <- combined_gene_metric_normal[conglomerate_genes,] # filtering the non-linearized normal splicing antigenicity by conglomerate and differential genes
...
test_1 <- combined_gene_metric_DA$DA==Inf & # which have zero normal splicing antigenicity
  !is.na(combined_gene_metric_DA$DA) # which are not NAs
test_2 <- combined_gene_metric_DA$DA!=Inf & # which have non-zero normal splicing antigenicity
  !is.na(combined_gene_metric_DA$DA) & # which are not NAs
  combined_gene_metric_DA$DA > 1 # which have DA greater than one (because no log transform)
test_3 <- combined_gene_metric_DA$DA==Inf & # which have zero normal splicing antigenicity
  !is.na(combined_gene_metric_DA$DA) # which are not NAs

combined_gene_metric_DA$DA[test_1] <- sample(combined_gene_metric_DA$DA[test_2],length(which(test_3)),replace=T) # replace those genes that have zero normal splicing antigenicity with sampled genes that have nonzero normal splicing antigenicity and therefore have a differential agretopicity score

test_1 <- combined_gene_metric_DA$DA_inv==Inf & # which have zero tumor splicing antigenicity
  !is.na(combined_gene_metric_DA$DA_inv) # which are not NAs
test_2 <- combined_gene_metric_DA$DA_inv!=Inf & # which have non-zero tumor splicing antigenicity
  !is.na(combined_gene_metric_DA$DA_inv) & # which are not NAs
  combined_gene_metric_DA$DA_inv > 1 # which have DA greater than one (becauase no log transform)
test_3 <- combined_gene_metric_DA$DA_inv==Inf & # which have zero tumor splicing antigenicity
  !is.na(combined_gene_metric_DA$DA_inv) # which are not NAs

combined_gene_metric_DA$DA[test_1]<-sample(combined_gene_metric_DA$DA[test_2],length(which(test_3)),replace=T)# replace those genes that have zero tumor splicing antigenicity with sampled genes that have nonzero tumor splicing antigenicity and therefore have a differential agretopicity score
combined_gene_metric_DA$genes <- rownames(combined_gene_metric_DA) # provide gene names for all differential agretopicity scores

#----------------------------------------------------#
# placing differential agretopicity data into container to be saved for later

splicing_antigenicity_DA_cohort <- combined_gene_metric_DA

#------------------------------------------------------------------------------#
# placing tumor splicing antigenicity data into container to be saved for later

splicing_antigenicity_tumor_genes_cohort <- data.frame(SA=unname(combined_gene_metric_tumor_mean),
...
splicing_antigenicity_tumor_samples_cohort <- data.frame(SA = apply(combined_gene_metric_tumor,2,mean),
                                                         cancer=rep(cancer,ncol(combined_gene_metric_tumor)),
                                                         type=rep("tumor",ncol(combined_gene_metric_tumor)),
                                                         sample=colnames(combined_gene_metric_tumor)) # calculating the average splicing antigenicity per sample across genes and marking with cancer subtype, tumor type, and sample name
...
splicing_antigenicity_normal_genes_cohort <- data.frame(SA=unname(combined_gene_metric_normal_mean),
...
splicing_antigenicity_normal_samples_cohort <- data.frame(SA = apply(combined_gene_metric_normal,2,mean),
                                                          cancer=rep(cancer,ncol(combined_gene_metric_normal)),
                                                          type=rep("normal",ncol(combined_gene_metric_normal)),
                                                          sample=colnames(combined_gene_metric_normal)) # calculating the average splicing antigenicity per sample across genes and marking with cancer subtype, tumor type, and sample name
...
high_DA_genes <-  combined_gene_metric_DA[combined_gene_metric_DA$DA>1,"genes"] # filtering out low DA genes, this was done incorrectly previously, I did not filter out low DA genes it as > 0, this entire section needs to be rerun
...
splicing_antigenicity_tumor_HIGH_DA_HIGH_CP_diff_sig<-combined_gene_metric_tumor[HIGH_DA_HIGH_CP_genes,] # saving the high differential agretopicity (DA), high coding potential (CP), and significantly differerential splicing antigenicity genes (between tumor and normal samples)
...
#------------------------------------------------------------------------------#
# preparing the splicing antigenicity for metadata annotations
...
splicing_antigenicity_per_sample$SA_PUR_div <- splicing_antigenicity_per_sample$splicing_antigenicity/splicing_antigenicity_per_sample$purity # calculate the tumor purity normalized splicing antigenicity per sample
...
cibersort_cols_to_add <- c("splicing_antigenicity","SA_PUR_div","purity","non_silent_mutations_per_mb","leukocyte_fraction","numberOfImmunogenicMutation","num_TCR_Clones","cancer")
...
a<-cor.test(splicing_antigenicity_per_sample$SA_PUR_div,log10(splicing_antigenicity_per_sample$non_silent_mutations_per_mb+1),method="kendall")
...
a<-cor.test(splicing_antigenicity_per_sample$SA_PUR_div,splicing_antigenicity_per_sample$leukocyte_fraction,method="kendall")
...
a<-cor.test(splicing_antigenicity_per_sample$SA_PUR_div,log10(splicing_antigenicity_per_sample$numberOfImmunogenicMutation+1),method="kendall")
...
a<-cor.test(splicing_antigenicity_per_sample$SA_PUR_div,log2(splicing_antigenicity_per_sample$num_TCR_Clones+1),method="kendall")
