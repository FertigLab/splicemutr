...
# compiling the separate mhcnuggets output files alleles
#path = results.output[0]
#kmer_files = os.listdir(path)  # a list of kmer_files with a fraction of the total unique number of kmers
#for file in kmer_files:
#    s = path + "{}" + "{}"
#    s = s.format("/", file)
#    s_split = s.split("/")  # parsing the path
#    kmer_file = s_split[len(s_split) - 1].split(".")[0]  # extracting just mhc file name
#    kmer = kmer_file.split("_")[0]
#    bash_comm = "{}" + "{}" + "{}" + "{}" + kmer_file + "{}" + "{}" + "{}" + "{}" + "{}" + "{}" + kmer + "{}"
#    bash_comm = bash_comm.format("cat", " ", path, "/", ".txt", " ", ">>", " ", path, "/", ".txt")
#    os.system(bash_comm) # this calls the cat append bash command
