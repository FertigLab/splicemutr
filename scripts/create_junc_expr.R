...
for (i in seq(1,total,100)){
  ...
  junc_expr <- list()
  juncs_all<-c()
  for (file in junc_files$V1[seq(start,end)]){
    print(file)
  ...
  saveRDS(junc_expr_comb,file=sprintf("%s/junc_expr_combined_%d.rds",out_dir,iter))
  saveRDS(junc_expr_comb_vst,file=sprintf("%s/junc_expr_combined_vst_%d.rds",out_dir,iter))
  iter<-iter+1
}
