...
rule analyze_splicemutr:
    ...
    shell:
        """
            ...
            do
                $SCRIPT_DIR/analyze_splicemutr.py -g {input.GENOTYPES} -s {input.SUMMARY_DIR} -d {input.SPLICE_DAT_FILE} -o {output.ANALYZE_SPLICEMUTR_OUT} -t {params.SUMMARY_TYPE} -n $VAR
            done
